...
void DeviceSetUp::SetGoodDevice()
{
		...
		DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&useAdapter)) != DXGI_ERROR_NOT_FOUND; ++i)
	{
		...
		{
			//採用したアダプタの情報をログに出力
			Log(DxCommon::debugLog, ConvertString(std::format(L"Use Adapter:{}\n", adapterDisc.Description)));
	...
	for (size_t i = 0; i < _countof(featureLevels); ++i)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			Log(DxCommon::debugLog, std::format("FeatureLevel:{}\n", featureLevelStrings[i]));
			break;
		}
	}
	...
	Log(DxCommon::debugLog, "Complete create D3D12Device\n");
